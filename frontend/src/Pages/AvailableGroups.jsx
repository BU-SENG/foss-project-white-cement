...
const recommendedGroups = [
  { match: 92, title: "Data Structures Practice", description: "Weekly review of core data structures. Focus on collaborative problem-solving.", members: { list: ['S', 'D'], extra: 2 } },
  { match: 88, title: "Calculus II Peer Tutors", description: "A friendly group for tackling tough Calculus concepts and homework together.", members: { list: ['C', 'T', 'P'], extra: 0 } },
    ...
    <div className="flex items-center">
      <div className="flex -space-x-2">
        {members.list.map((initial, index) => (
          <div
            key={index}
            className={`flex items-center justify-center ${sizeClass} font-bold rounded-full bg-gray-700 ring-2 ring-gray-900 ${index > 0 ? overlapClass : ''}`}
          >
            {initial}
          </div>
        ))}
        {members.extra > 0 && (
          <div className={`flex items-center justify-center ${sizeClass} font-bold rounded-full bg-blue-600 ring-2 ring-gray-900 ${overlapClass}`}>
            +{members.extra}
          </div>
        )}
      </div>
      {isLarge && <span className="text-gray-400 ml-4">Members ({members.count})</span>}
    </div>
  );
};
  ...
  <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-xl flex flex-col lg:flex-row justify-between items-start lg:items-center">
    ...
    <div className="flex-1 min-w-0 mb-4 lg:mb-0">
      <div className="flex items-center mb-2 space-x-3">
        ...
        <span className="text-green-400 text-sm font-semibold">{data.match}% Match</span>
      </div>
      
      <h3 className="text-2xl font-bold text-white mb-2">{data.title}</h3>
      <p className="text-gray-400 text-base mb-4 lg:mb-6 max-w-2xl">{data.description}</p>
